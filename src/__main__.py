"""CLI entry point so the engine runs with ``uv run python -m src``."""
...
def main() -> None:
    ...
    Raises:
        SystemExit: Exits with code 1 if parsing, file I/O, or errors occur.
    """
    try:
        parser = Parser()
        parsed_data = parser.parse_and_load()
        if parsed_data is None:
            raise ValueError("Something went wrong during parsing.")
